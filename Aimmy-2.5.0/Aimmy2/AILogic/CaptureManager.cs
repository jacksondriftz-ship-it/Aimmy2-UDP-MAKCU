...
namespace AILogic
{
    internal class CaptureManager
    {
        ...
        // UDP Stream
        private UDPVideoReceiver? _udpReceiver;
        private static UDPVideoReceiver? _staticUdpRef; // static mirror so UDPStatus property can access it
        ...
        // Read by SettingsMenuControl's status timer via AILogic.CaptureManager.UDPStatus
        public static string UDPStatus => _staticUdpRef?.StatusMessage ?? "Not connected";

        private Bitmap? UDPStream(Rectangle detectionBox)
        {
            // Lazily start the receiver the first time we enter this method,
            // or restart it if the IP/port settings changed since last time.
            // Keys match exactly what SettingsMenuControl writes into Dictionary.dropdownState.
            string ip   = Dictionary.dropdownState.TryGetValue("UDP Stream IP",   out var rawIp)   ? rawIp   : "0.0.0.0";
            int    port = 11000;
            if (Dictionary.dropdownState.TryGetValue("UDP Stream Port", out var rawPort))
                int.TryParse(rawPort, out port);

            if (_udpReceiver == null || !_udpReceiver.IsConnected ||
                _udpReceiver.ListenIP != ip || _udpReceiver.ListenPort != port)
            {
                _udpReceiver?.Disconnect();
                _udpReceiver?.Dispose();
                _udpReceiver = new UDPVideoReceiver();
                bool ok = _udpReceiver.Connect(ip, port);
                _staticUdpRef = _udpReceiver;
                LogManager.Log(ok ? LogLevel.Info : LogLevel.Error,
                    $"UDP Stream receiver {(ok ? "started" : "failed to start")} on {ip}:{port}");
            }

            // GetLatestFrame() returns a cloned Bitmap ready for the AI pipeline,
            // or null if no frame has arrived yet – the AI loop already handles null.
            Bitmap? frame = _udpReceiver.GetLatestFrame();

            // If the frame size doesn't match the detection box, scale it.
            // (The sender crop should already match, but this is a safety net.)
            if (frame != null &&
                (frame.Width != detectionBox.Width || frame.Height != detectionBox.Height))
            {
                var scaled = new Bitmap(detectionBox.Width, detectionBox.Height, PixelFormat.Format32bppArgb);
                using (var g = Graphics.FromImage(scaled))
                    g.DrawImage(frame, 0, 0, detectionBox.Width, detectionBox.Height);
                frame.Dispose();
                frame = scaled;
            }
        ...
        public Bitmap? ScreenGrab(Rectangle detectionBox)
        {
            ...
            if (selectedMethod != _currentCaptureMethod)
            {
                ...
                else if (selectedMethod == "UDP Stream")
                {
                    DisposeDxgiResources();
                    // UDP receiver is started lazily inside UDPStream() below
                }
                else // DirectX
                {
                    StopUDPReceiver();
